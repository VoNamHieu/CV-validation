"""Per-company / per-platform ATS adapters + the `_ADAPTERS` registry.

Each adapter is a `_is_x(url[, html])` detector + a `_x(...)` fetcher. To add
one: write the pair here and append a line to `_ADAPTERS` at the bottom — the
only shared edit point. Shared helpers come from `._shared`.
"""
